---
slug: /@jitsi/excalidraw/api/utils
---

# Utils

These are pure Javascript functions exported from the @jitsi/excalidraw [`@jitsi/excalidraw`](https://npmjs.com/@jitsi/excalidraw). If you want to export your drawings in different formats eg `png`, `svg` and more you can check out [Export Utilities](/docs/@jitsi/excalidraw/API/utils/export). If you want to restore your drawings you can check out [Restore Utilities](/docs/@jitsi/excalidraw/API/utils/restore).
...
```js
import { serializeAsJSON } from "@jitsi/excalidraw";
```
...
```js
import { serializeLibraryAsJSON } from "@jitsi/excalidraw";
```
...
```js
import { isInvisiblySmallElement } from "@jitsi/excalidraw";
```
...
```js
import { loadFromBlob } from "@jitsi/excalidraw";
...
```js
import { loadLibraryFromBlob } from "@jitsi/excalidraw";
```
...
```js showLineNumbers
import { loadSceneOrLibraryFromBlob, MIME_TYPES } from "@jitsi/excalidraw";
...
```js
import { getFreeDrawSvgPath } from "@jitsi/excalidraw";
```
...
```js
import { isLinearElement } from "@jitsi/excalidraw";
```
...
```js
import { getNonDeletedElements } from "@jitsi/excalidraw";
```
...
```js
import { mergeLibraryItems } from "@jitsi/excalidraw";
```
...
```js
import { parseLibraryTokensFromUrl } from "@jitsi/excalidraw";
```
...
```js
import { useHandleLibrary } from "@jitsi/excalidraw";
...
```js
import { getSceneVersion } from "@jitsi/excalidraw";
```
...
```js
import { sceneCoordsToViewportCoords } from "@jitsi/excalidraw";
```
...
```js
import { viewportCoordsToSceneCoords } from "@jitsi/excalidraw";
```
...
```js
import { defaultLang, languages, useI18n } from "@jitsi/excalidraw";
```
...
List of supported language codes. You can pass any of these to `Excalidraw`'s [`langCode` prop](/docs/@jitsi/excalidraw/api/props/#langcode).
...
```js
import { getCommonBounds } from "@jitsi/excalidraw";
```
...
```js
import { elementsOverlappingBBox } from "@jitsi/excalidraw";
```
...
```js
import { isElementInsideBBox } from "@jitsi/excalidraw";
```
...
```js
import { elementPartiallyOverlapsWithOrContainsBBox } from "@jitsi/excalidraw";
```
